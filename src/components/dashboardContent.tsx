...
const DashboardContent = async ({
	...
	const lastMonthIncome = income.find((item) => item.name === 'Last Month')?.income;
	const thisMonthIncome = income.find((item) => item.name === 'This Month')?.income;
			...
			>
				...
				>
									...
									{Number(lastMonthIncome) > Number(thisMonthIncome) ? (
										<span>
											...
											<span style={{ color: 'red' }}>
												{calculatePercent(thisMonthIncome ?? 0, lastMonthIncome ?? 0)}%
											</span>
										</span>
									) : (
					...
					<Paper sx={{ p: 2, backgroundColor: 'white', marginTop: '20px' }}>
						...
						>
							<Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
								<PieChartWithPaddingAngle booksCategory={numberOfBooksByCategory} />
							</Box>
							<Box
								sx={{
									width: '100%',
